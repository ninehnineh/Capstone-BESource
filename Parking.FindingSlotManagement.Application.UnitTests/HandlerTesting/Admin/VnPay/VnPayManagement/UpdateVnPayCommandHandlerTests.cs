...
namespace Parking.FindingSlotManagement.Application.UnitTests.HandlerTesting.Admin.VnPay.VnPayManagement
{
    public class UpdateVnPayCommandHandlerTests
    {
        ...
        public async Task UpdateVnPayCommandHandler_Should_Update_VnPayInfor_Successfully()
        {
            ...
            _accountRepositoryMock.Setup(x => x.GetItemWithCondition(x => x.RoleId == 1 && x.IsActive == true && x.IsCensorship == true && x.UserId.Equals(request.ManagerId), null, true)).ReturnsAsync(checkManagerExist);
            _vnpayRepositoryMock.Setup(x => x.GetItemWithCondition(x => x.ManagerId.Equals(checkManagerExist.UserId), null, true)).ReturnsAsync((Domain.Entities.VnPay)null);
            ...
            OldVnPayInfor.ManagerId.ShouldBe(request.ManagerId);
        ...
        public async Task UpdateVnPayCommandHandler_Should_Return_Not_Found_If_Manager_Does_Not_Exist()
        {
            ...
            _vnpayRepositoryMock.Setup(x => x.GetById(request.VnPayId))
                .ReturnsAsync(OldVnPayInfor);
            _accountRepositoryMock.Setup(x => x.GetItemWithCondition(x => x.RoleId == 1 && x.IsActive == true && x.IsCensorship == true && x.UserId.Equals(request.ManagerId), null, true)).ReturnsAsync((User)null);
